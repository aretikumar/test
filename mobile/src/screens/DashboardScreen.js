...
import { C } from '../theme';
import { getSettings, saveSettings, getJobs } from '../storage';
...
export default function DashboardScreen() {
  ...
  const checkNow = async () => {
    ...
    try {
      ...
      await load();
      Alert.alert('Check Complete', `Checked ${result.checked} jobs, found ${result.found} new matches`);
    ...
    <ScrollView style={s.wrap} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={C.accent} />}>
      ...
      <View style={[s.banner, { borderColor: settings.monitoring_enabled ? C.success : C.danger }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          {settings.monitoring_enabled && <Text style={s.bannerSub}>Every {settings.check_interval_minutes || 5} min · Part-time only</Text>}
        </View>
